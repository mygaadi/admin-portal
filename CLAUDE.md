...
| Service Stations | **Real** (2026-08-16), location fields temporarily disabled | Create/edit only collects name/manager/phone/email/capacity for now — location handling is mid-rework, see Product-direction deviations. `managerId` picker still `MOCK_MANAGERS` (Open Question #2). "My Station" (Station Manager) now resolves its own station via `useMyStation()` (`?managerId=`) instead of a fixed mock id; its inventory sub-section stays mock |
...
- **Service Station location handling is being reworked, mid-flight (2026-08-16).** `locationId` isn't required by the backend (`ServiceStationRequest.locationId` has no `@NotNull`, `service_station.location_id` is a nullable FK), so a first pass wired a location-creation-first flow (`POST`/`PUT /api/users/me/locations` to get a `locationId`, then create/update the station with it). User has since asked the backend dev for something better: **`POST`/`PUT /api/service-stations` accepting the location fields (address/city/state/lat/lng) inline**, instead of requiring a separate location row up front. Until that lands, the location picker and all location-related code in `service-station-schema.ts`/`service-stations-api.ts`/`service-station-form-dialog.tsx` is commented out (not deleted) — the create/edit station form only collects name/manager/phone/email/capacity for now, stations are created with `locationId: null`. `service-stations-page.tsx` and `my-station-page.tsx` guard against the resulting null city/state/addressLine so they don't render literal "null, null" text. Once the backend dev ships the inline shape, re-enable the picker and wire it to send location fields directly in the single station request — a simpler integration than the two-step flow that was removed. `managerId` is still a raw Long with no listing endpoint on the backend (see Open Questions #2) — `MOCK_MANAGERS` stands in for now.
