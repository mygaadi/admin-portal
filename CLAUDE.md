...
| Service Stations | **Real** (2026-08-16) | Location-creation-first flow: `POST`/`PUT /api/users/me/locations` for the address, then the station references the resulting `locationId`. `managerId` picker still `MOCK_MANAGERS` (Open Question #2). "My Station" (Station Manager) now resolves its own station via `useMyStation()` (`?managerId=`) instead of a fixed mock id; its inventory sub-section stays mock |
...
- **Service Station's `locationId` is unchanged in the real API** (still a raw FK to a separate `location` table, itself tied to the creating user via `/api/users/me/locations` — PRD §10/§6 model, confirmed accurate) — **now wired**: `location-picker.tsx` captures `state` from the Nominatim result, and `service-stations-api.ts` creates/updates the `location` row first (`POST`/`PUT /api/users/me/locations`) before creating/updating the station with the resulting `locationId`. `managerId` is still a raw Long with no listing endpoint on the backend (see Open Questions #2) — `MOCK_MANAGERS` stands in for now; once Accounts is wired to the real API, consider sourcing this picker from station managers created in the current session instead.
